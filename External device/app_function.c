...
#define PDR_INTERVAL 100000

#define RTT_INTERVAL 100000

#define RSSI_INTERVAL 100000
...
void amcomPacketHandler(const AMCOM_Packet* packet, void* userContext){
    ...
    switch (packet->header.type) {
        ...
        case AMCOM_RTT_RESPONSE:
            ...
            gettimeofday(&rtt_information[rtt_response.test_number].rtt_info_time[rtt_response.packet_number].stop, NULL);
            for(int i = 0; i < AMCOM_MAX_DEVICE_NAME_LEN; i++){
                rtt_response.deviceName [i] = packet->payload[i];
            }
            
            long long int rtt_temp = (rtt_information[rtt_response.test_number].rtt_info_time[rtt_response.packet_number].stop.tv_sec - rtt_information[rtt_response.test_number].rtt_info_time[rtt_response.packet_number].start.tv_sec)*1000000
            +(rtt_information[rtt_response.test_number].rtt_info_time[rtt_response.packet_number].stop.tv_usec-rtt_information[rtt_response.test_number].rtt_info_time[rtt_response.packet_number].start.tv_usec);
            
            printf(" Czas rtt : %lld us\n", rtt_temp);
            sprintf(results, "RTT,%d,%d,%lld",rtt_response.test_number,rtt_response.packet_number,rtt_temp);
        ...
        case AMCOM_TON_RESPONSE:
            ...
            ton_response.time_on = packet->payload[AMCOM_MAX_DEVICE_NAME_LEN]|packet->payload[AMCOM_MAX_DEVICE_NAME_LEN+1]<<8|
                                    packet->payload[AMCOM_MAX_DEVICE_NAME_LEN+2]<<16|packet->payload[AMCOM_MAX_DEVICE_NAME_LEN+3]<<24;
        ...
        case AMCOM_THROUGHPUT_RESPONSE:
            ...
            thr_response.packet_size = packet->payload[AMCOM_MAX_DEVICE_NAME_LEN+2];
            printf("Recv pack %d \n", thr_response.recv_packets);
            sprintf(results, "THR,%d,%d",thr_response.packet_size,thr_response.recv_packets);
...
void thr_test(const int packet_size, const int dev_iterator){
    ...
    thr_start.expect_packet_size = packet_size+5; // ADD 5B -> SOP,TYPE,LENGTH,CRC
...
void send_thr(uint16_t number_of_packets[], size_t num_size, const void *thr_start, const void *thr_req, const int dev_iterator){
    ...
    for(int n = 0; n < num_size; n++){       
        ...
        for (int i=0; i < number_of_packets[n]; i++){
            usleep(1000000*10/number_of_packets[n]); // 10*1000000us = 10s  Interval = 10/number of packets
